---
title: "Supporting information - An experimental study of language evolution in heterogeneous populations"
subtitle: "Full analyses and plots"
author: "Mathilde Josserand (mathilde.josserand@gmail.com)"
date: "`r date()`"
output:
  html_document: 
    highlight: textmate
...
# Figure and Table caption adapted from Dan Dediu's code (not sure which one) who get inspired by: https://stackoverflow.com/questions/37116632/rmarkdown-html-number-figures: 
...
# enter path file
pathfiles = "C:/Users/Mathilde/OneDrive - Université Laval/Documents/Travail/Projets/Doctorat/ExperimentalChains/ExpCommunityVariation/InputFiles/Data/"
...
# enter path file
pathfiles = "C:/Users/Mathilde/OneDrive - Université Laval/Documents/Travail/Projets/Doctorat/ExperimentalChains/ExpCommunityVariation/InputFiles/Other/"
...
**If you want to replicate the experiment, you will find the code for the experiment and also a file (`How to conduct the experiment.docx`) explaining steps by step how to proceed in the folder `Program_Experiment` in my [github: mathjoss/ExpCommunityVariation](https://github.com/mathjoss/ExpCommunityVariation).**

## Participants

`r nrow(df_other)` participants participated in this study (`r nrow(df_other[df_other$Gender=="F",])` women and `r nrow(df_other[df_other$Gender=="M",])` men). They were between the ages of 18 and 50 (M= `r round(mean(df_other$Age))`, SD=`r round(sd(df_other$Age))` ). Please refer to [Participants' characteristics](#participants-characteristics) to see more about the distribution of age, gender, and other measures.
...
After each group passation, we use `CleanUpFiles.R` (see folder `InputFiles` of my github folder [github: mathjoss/ExpCommunityVariation](https://github.com/mathjoss/ExpCommunityVariation)) to perform the following steps:
...
```{r, fig.cap=capFig("Learning success (accuracy on the left, initial production similarity on the right) per group type. Dots indicate each participant. The red dot shows the mean of the group, and red point range shows the standard error."), fig.width=10, fig.height=5}
...
Participants remembered in average `r round(mean(df_agg_acc$mean_acc),2)`% of the initial labels, with a standard deviation of `r round(sd(df_agg_acc$mean_acc),2)`. The average production similarity with the initial labels is of `r round(mean(df_agg_dist2$mean_lev_dis),2)` with a standard-deviation of `r round(sd(df_agg_dist2$mean_lev_dis),2)`.
...
```{r, fig-learning-shape, fig.cap=capFig("Levenshtein distance between initial words and initial productions for each word. Blue dots show biased participants."), fig.width=12, fig.height=6}
...
```{r, fig-part-charact, fig.cap=capFig("Distribution of the scores. Red shows the results for the control group, blue shows the results for the heterogenous group."), fig.width=10, fig.height=6}
...
```{r, fig-comsucess-time-groups, fig.cap=capFig("Evolution of communicative success for each group."), fig.width=9, fig.height=7}
...
```{r, fig-comsucess-time-agg , fig.cap=capFig("Evolution of communicative success aggregated by group type: control or heterogenous."), fig.width=8, fig.height=5}
...
```{r}
# New model 1 with no aggregation by pair
df2 <- df %>%
  mutate(Round2 = ifelse(Round == 9, 0, ifelse(Round==8, 1, ifelse(Round==7, 2, ifelse(Round==6, 3, ifelse(Round==5, 4, ifelse(Round==4, 5, ifelse(Round==3, 6, ifelse(Round==2, 7, ifelse(Round==1, 8, NA)))))))))) 

model <- glmer(ACC ~ GroupType * Round2 + (1 | GroupID) + (1 | pair), data = df2, family=binomial(link="logit"))
summary(model)
...
```{r,  fig-comsucess-split-hetero , fig.cap=capFig("Same as above, except that here we split heterogenous groups in 2: pairs including the biased participant and pairs without the biased participant."), fig.width=6, fig.height=6}
...
```{r, fig-comsuccess-pointrange, fig.cap=capFig("Mean communicative success for each group, whether unbiased communicate with biased participant (part 1) or unbiased participant (part 2, 3, 4) Bars show standard error."), fig.width=8, fig.height=5}
...
df_agg <- df %>%
  mutate(ACC=ACC*100) %>%
  filter(TypeTest == "ComGame" ) %>%
  mutate(InteractionNum = ((index - 25) %% 16) + 1) %>%
  dplyr::group_by(InteractionNum, GroupType) %>%
  dplyr::summarize(mean_acc = mean(ACC, na.rm=TRUE)) 
  
# plot
ggplot(df_agg, aes(x=InteractionNum, y=mean_acc, color=GroupType, group=GroupType)) +
  geom_point() +
  geom_line() +
  geom_smooth(method="lm") +
  labs(x="Interaction number (within a round)", y="Mean Communicative Success")  +
...
summary(lmer(ACC ~ InteractionNum*GroupType + (1 | Producer) + (1 | GroupID) + (1 | Round), data=df_agg))

#cor.test(df_agg$InteractionNum, df_agg$mean_acc, method="spearman")
...
```{r, fig-evolution-convergence, fig.cap=capFig("Evolution of convergence with time for each group."), fig.width=10, fig.height=8}
...
```{r, fig-evolution-convergence33, fig.cap=capFig("Same, but aggregated by group."), fig.width=6, fig.height=6}
...
# model
model <- lmer(All ~ GroupType * Round2 + (1 | GroupID), data = df_agg)
summary(model)
...
```{r, fig-evolution-convergence-agg, fig.cap=capFig("Same as above, but distinguishing between hetero with biased and hetero without biased participants."), fig.width=6, fig.height=6}
...
# model
model <- lmer(mean_lev_dist ~ TypeGroup*Round2 + (1 | GroupID), data=df_agg)
summary(model)
...
```{r, fig-evolution-ProducSim-sha, fig.cap=capFig("Evolution of the production similarity at each round, for each shape. Please note that this plot does not include the productions of the biased participant."), fig.width=10, fig.height=8}
...
```{r, fig-evolution-ProducSim, fig.cap=capFig("Evolution of the production similarity at each round, for each shape. Please note that this plot does not include the productions of the biased participant."), fig.width=10, fig.height=8}
...
```{r, fig-evolution-ProducSim-agg-group, fig.cap=capFig("Same plot as above, but aggregated by shape. Please note that this plot does not include the productions of the biased participant."), fig.width=9, fig.height=6}
...
```{r, fig-evolution-ProducSim-distinct, fig.cap=capFig("Same plot as above, except that here, we differentiate between pairs interacting with the biased individuals, and pairs interacting without the biased individual."), fig.width=6, fig.height=6}
...
```{r, fig-evolution-SimBiased-distinct, fig.cap=capFig("Evolution of the production similarity of unbiased participants with the biased participants."), fig.width=8, fig.height=6}
...
p <- p + geom_point(size=5) +
  geom_line(size=1) +
  facet_grid(GroupType ~ .) +
  labs(y="Prod sim with biased", fill="GroupNum") +
  scale_x_continuous(breaks = c(0:10)) +
  scale_color_viridis_d()
p
...
```{r, fig-evolution-SimBiased-distinct2, fig.cap=capFig("Same as above, but plotting the linear regression going through the points."), fig.width=6, fig.height=6}
...
```{r, fig-evolution-stability-with, fig.cap=capFig("Evolution of stability with time for the group (including biased participants."), fig.width=10, fig.height=8}
...
```{r, fig-evolution-stability-without, fig.cap=capFig("Evolution of stability with time for the group (excluding biased participants)."), fig.width=8, fig.height=6}
...
```{r, fig-evolution-entropy-without-agg, fig.cap=capFig("Evolution of stability aggregated with time for each type of group: everyone (all) or everyone except the biased participant (Without Biased)."), fig.width=10, fig.height=5}
...
```{r, fig-freq-round-all, fig.cap=capFig("Plot showing the evolution of frequency of biased letters in control groups (red) and heterogenous groups (blue). Each line represent a group, and the thick line shows the linear regression applied to all these groups."), fig.width=8, fig.height=6}
...
```{r, fig-freq-round-all-loess, fig.cap=capFig("Same as above, but using loess regression."), fig.width=8, fig.height=6}
...
## model with random slopes for round type
...
```{r, fig-freq-round-all-agg, fig.cap=capFig("Same plot as above, aggregated by group number."), fig.width=8, fig.height=6}
...
```{r, fig-freq-testing2, fig.cap=capFig("Change in the frequency of biased and unbiased letters in the first testing (before the communication game) and in the second testing (after the communication game) at a group-level. Each point represent a group, and the thin grey line indicate the within design (each group is tested before and after)."), fig.width=7, fig.height=5}
...
```{r, bootstrap, fig.cap=capFig("Estimate of the confident intervals from the Bootstrapping technique. ."), fig.width=9, fig.height=5}
...
```{r, fig-acc-group2-and6, fig.cap=capFig("Investigating more the relation between adaptability and performance at learning the initial words. Here, we look at the initial accuracy (binary, 0 or 1) and the initial distance words (levenshtein distance) in the first testing for each group. We expect the performance to be better for Group 2 and Group 6."), fig.width=10, fig.height=9}
...
```{r, fig-adapt-learning, fig.cap=capFig("Investigating more the relation between adaptability and performance at learning the initial words. Each point represent a participant, and the blue line shows the linear regression between all these points."), fig.width=6, fig.height=4}
...
```{r, fig-evolution-ProducSim_group, fig.cap=capFig("Evolution of the production similarity at each round and for each group. Please note that this plot does not include the productions of the biased participant."), fig.width=6, fig.height=5}
...
```{r, fig-freq-round-indiv-all, fig.cap=capFig("Plot showing the evolution of frequency of biased letters in control groups (red) and heterogenous groups (blue). Each line represent a group, and the thick line shows the linear regression applied to all these groups."), fig.width=8, fig.height=6}
...
```{r, fig-freq-indv, fig.cap=capFig("Evolution of the mean frequency of biased  and unbiased letters for each individual in each group type."), fig.width=7, fig.height=5}
...
```{r, fig-freq-testing, fig.cap=capFig("Change in the frequency of biased and unbiased letters in the first testing (before the communication game) and in the second testing (after the communication game) at a group-level. Each point represent a group, and the thin grey line indicate the within design (each group is tested before and after)."), fig.width=12, fig.height=7}
...
```{r, fig-3-chapter, fig.cap=capFig("Evolution of the mean frequency of biased letters comparing two types of pair: pair including the biased participant (1-2, 1-3, and 1-4) and pair excluding the biased participant (2-3, 3-4, 2-4)."), fig.width=7, fig.height=5}
...
```{r, fig-freq-pair2, fig.cap=capFig("Evolution of the mean frequency of biased letters comparing two types of pair: pair including the biased participant (1-2, 1-3, and 1-4) and pair excluding the biased participant (2-3, 3-4, 2-4)."), fig.width=6, fig.height=6}
...
```{r, fig-corr-adapt, fig.cap=capFig("Correlation between the adaptability scores."), fig.width=7, fig.height=6}
...
```{r, fig-distrib-var, fig.cap=capFig("Distribution of the scores. Red shows the results for the control group, blue shows the results for the heterogenous group."), fig.width=10, fig.height=6}
...
```{r, fig-corr-predictors, fig.cap=capFig("Correlation between the predictors."), fig.width=12, fig.height=11}
...
```{r, fig-adapt1, fig.cap=capFig("Correlations between the different individual measures and our score for Adaptability1. Reminder: in the dictator game 1 is keeping all the money for oneself, 5 is to share it all."), fig.width=10, fig.height=8}
...
```{r, fig-adapt2, fig.cap=capFig("Correlations between the different individual measures and our score for Adaptability2. Reminder: in the dictator game 1 is keeping all the money for oneself, 5 is to share it all."), fig.width=10, fig.height=8}
...
```{r, fig-adapt3, fig.cap=capFig("Correlations between the different individual measures and our score for Adaptability3. Reminder: in the dictator game 1 is keeping all the money for oneself, 5 is to share it all."), fig.width=10, fig.height=8}
...
```{r, fig-adapt4, fig.cap=capFig("Correlations between the different individual measures and our score for Adaptability4. Reminder: in the dictator game 1 is keeping all the money for oneself, 5 is to share it all."), fig.width=10, fig.height=8}
...
#### Other analysis: predictors, adaptability
...
```{r}
# merge df with adaptation score and initial df, to add the value of accuracy
df_adapt5 <- merge(df_adapt4, df, by=c("GroupNum", "GroupType", "Round", "Producer", "Shape", "Word"), all.x=T, all.y=F)

df_cor <- df_adapt5 %>%
  filter(Round != 0) %>%
  dplyr::group_by(GroupType, GroupNum, Producer) %>%
  dplyr::summarize(Adaptability4 = mean(adapt4),
                   CommunicativeSuccess = mean(ACC)) %>%
  mutate(GroupID = paste(GroupType, GroupNum, sep="_"))

ggplot(df_cor, aes(x=CommunicativeSuccess, y=Adaptability4)) +
  geom_point() +
  geom_smooth(method="lm") +
  geom_smooth()

#test <- lm(df_cor$CommunicativeSuccess ~ df_cor$Adaptability4)
#test_quad <- lm(df_cor$CommunicativeSuccess  ~ poly(df_cor$Adaptability4, 2))

correlation_test <- cor.test(df_cor$CommunicativeSuccess, df_cor$Adaptability4, method = "spearman")

# df_cor <- df_adapt5 %>%
#   filter(Round != 0) %>%
#   mutate(GroupID = paste(GroupType, GroupNum, sep="_"))
# 
# summary(lmer(ACC ~ adapt4 + (1 | GroupID), data=df_cor))
...
```{r}
cor.test(df_cor$CommunicativeSuccess, df_cor$Adaptability4, method = "pearson")
cor.test(df_cor$CommunicativeSuccess, df_cor$Adaptability4, method = "spearman")
```
...
df_prod <- df %>%
  filter(TypeTest=="ComGame") %>%
  group_by(Producer, GroupType, GroupNum) %>%
  dplyr::summarize(ComSuccess_Prod = mean(ACC, na.rm=T)) %>%
  dplyr::rename(PartID = Producer)
df_guess <- df %>%
  filter(TypeTest=="ComGame") %>%
  group_by(Guesser, GroupType, GroupNum) %>%
  dplyr::summarize(ComSuccess_Guesser = mean(ACC, na.rm=T)) %>%
  dplyr::rename(PartID = Guesser)

df_both <- merge(df_prod, df_guess, by=c("PartID", "GroupType", "GroupNum"), all.x=T, all.y=T)
df_both$MeanComSuccess = (df_both$ComSuccess_Guesser + df_both$ComSuccess_Prod)/2

df_both2 <- merge(df_both, df_other, by=c("PartID", "GroupType", "GroupNum"), all.x=T, all.y=T)

p1 <- ggplot(df_both2, aes(x=prosoc, y=MeanComSuccess)) +
  geom_point()+
  geom_smooth(method="lm") +
  labs(x="Prosociality", y="Communicative Success")

p2 <- ggplot(df_both2, aes(x=DictatorGame, y=MeanComSuccess)) +
  geom_point()+
  geom_smooth(method="lm") +
  labs(x="DictatorGame", y="Communicative Success")

ggarrange(p1, p2, ncol=2)

# model with aggregated data
df_both2$GroupID <- paste(df_both2$GroupType, df_both2$GroupNum, sep="_")
summary(lmer(prosoc ~ MeanComSuccess + (1 | GroupID), data=df_both2))
summary(lmer(DictatorGame ~ MeanComSuccess + (1 | GroupID), data=df_both2))

# model with data the least aggregated as possible

df2 <- gather(df, typepart, PartID, Producer:Guesser)
df3 <- merge(df2, df_other[,c("PartID", "prosoc", "DictatorGame", "GroupType", "GroupNum")], by=c("PartID", "GroupType", "GroupNum"))
summary(lmer(prosoc ~ ACC + (1 | PartID) + (1 | GroupID), data=df3))
summary(lmer(DictatorGame ~ ACC + (1 | PartID) + (1 | GroupID), data=df3))
...
```{r, fig-1-chapter, fig.cap=capFig("Figure 1 paper"), fig.width=12, fig.height=5}
...
```{r, fig-2-chapter-bis, fig.cap=capFig("Figure 2 paper."), fig.width=11, fig.height=5}
...
```{r, fig-2-chapter, fig.cap=capFig("Figure 2 paper."), fig.width=11, fig.height=5}
