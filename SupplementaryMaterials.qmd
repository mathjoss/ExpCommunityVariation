...
After each group passation, we use `CleanUpFiles.R` to perform the following steps:
...
There is a significant improvement in performance with time in both conditions (control and heterogenous), as shown by the main effect of the Round2 variable. A **lower performance is observed in the heterogenous groups** than in the control group, as shown by the main effect of the GroupType variable depsite the very high performance reached by heterogenous Group 2. Moreover, no interaction effect between group type and round is present.
...
#| message: false
#| warning: false
#| label: fig-evolution-ProducSim-agg
...
model <- lmer(Adaptability1 ~ prosoc + WorkingMem + CogFlexibility + DictatorGame + Age + Gender + AccLearning + ProducSim + (1 | GroupNum), data = df_other2)
summary(model)
r.squaredGLMM(model)
...
model <- lmer(Adaptability2 ~ prosoc + WorkingMem + CogFlexibility + DictatorGame + Age + Gender + AccLearning + ProducSim + (1 | GroupNum), data = df_other2)
summary(model)
r.squaredGLMM(model)
...
# Model adaptability 3
model <- lmer(Adaptability3 ~ prosoc + WorkingMem + CogFlexibility + DictatorGame + Age + Gender + AccLearning + ProducSim + (1 | GroupID), data = df_other)
summary(model)
r.squaredGLMM(model)
...
```{r}
model <- lmer(Adaptability3 ~ prosoc + WorkingMem + CogFlexibility + DictatorGame + Age + Gender + AccLearning + ProducSim + (1 | GroupID), data = df_other[!(df_other$PartID==1 & df_other$GroupType=="Hetero"),])
summary(model)
r.squaredGLMM(model)
...
model <- lmer(Adaptability4 ~ prosoc + WorkingMem + CogFlexibility + DictatorGame + Age + Gender + AccLearning + ProducSim + (1 | GroupID), data = df_other)
summary(model)
r.squaredGLMM(model)
car::vif(model)
...
model <- lmer(Adaptability4 ~ prosoc + WorkingMem + CogFlexibility + DictatorGame + Age + Gender + AccLearning + ProducSim + (1 | GroupID), data = df_other[!(df_other$PartID==1 & df_other$GroupType=="Hetero"),])
summary(model)
r.squaredGLMM(model)
car::vif(model)
...
# 
# model <- lmer(Adaptability4 ~ prosoc_z + WorkingMem_z + CogFlexibility_z + DictatorGame_z + Age_z + Gender + AccLearning_z + ProducSim_z + (1 | GroupID), data = df_other_z)
# summary(model)
# r.squaredGLMM(model)
